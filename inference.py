...
for fold_idx, ckpt_path in enumerate(checkpoint_paths):
    ...
    fold_probs = []
    with th.no_grad():
        for i in tqdm(range(0, len(texts), batch_size), desc=f'Fold {fold_idx + 1}/{len(checkpoint_paths)}', unit='batch'):
