...
parser.add_argument('--sentiment', type=str, default=None,
                    help='Filter by sentiment. '
                         'Predictions CSV: positive, negative, neutral. '
                         'Labeled CSV: 0, 1, 2. '
                         'Omit for all sentiments.')
parser.add_argument('--month', type=str, default=None,
                    help='Filter by month e.g. 2024-01. Omit for all months.')
parser.add_argument('--batch_size', type=int, default=64)
parser.add_argument('--max_length', type=int, default=128)
parser.add_argument('--device', type=str, default='cuda')
parser.add_argument('--output_dir', type=str, default='topic_modeling_results')
...
def plot_monthly_sentiment(df, output_dir):
    ...
    plt.tight_layout()
    path = os.path.join(output_dir, 'monthly_sentiment_trend.png')
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\nMonthly sentiment trend saved to {path}")

    monthly_csv = os.path.join(output_dir, 'monthly_sentiment_counts.csv')
...
if args.sentiment:
    ...
    # accept both '0' and 'positive'
    if sentiment_filter in ['0', '1', '2']:
        sentiment_filter = label_map_int[int(sentiment_filter)]
...
sentiment_tag = args.sentiment or 'all'
month_tag     = args.month     or 'all_months'
output_tag    = f'{sentiment_tag}_{month_tag}'
...
plt.title(f'UMAP — Topic Clusters ({output_tag})', fontsize=13)
umap_path = os.path.join(args.output_dir, f'umap_{output_tag}.png')
...
# ── Word clouds ───────────────────────────────────────────────────────────────
def generate_wordclouds(llr_results, output_dir, output_tag, n_topics, colors):
    ...
    for c in range(n_topics):
        ...
        path = os.path.join(output_dir, f'wordcloud_cluster{c}_{output_tag}.png')
        wc.to_file(path)
    ...
    plt.suptitle(f'Topic Word Clouds ({output_tag})', fontsize=15, fontweight='bold')
    plt.tight_layout()
    combined_path = os.path.join(output_dir, f'wordclouds_combined_{output_tag}.png')
    plt.savefig(combined_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Combined word cloud grid saved to {combined_path}")


generate_wordclouds(llr_results, args.output_dir, output_tag, args.n_topics, colors)

# ── Save outputs ──────────────────────────────────────────────────────────────
clusters_csv = os.path.join(args.output_dir, f'tweets_with_clusters_{output_tag}.csv')
df.to_csv(clusters_csv, index=False)
print(f"\nCluster assignments saved to {clusters_csv}")

keywords_json = os.path.join(args.output_dir, f'llr_keywords_{output_tag}.json')
